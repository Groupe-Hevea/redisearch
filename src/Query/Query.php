...
class Query
{
    ...
    /**
     * This methods, adds filter for tag fields.
     *
     * @param string $fieldName
     * @param array $tags
     *
     * @return $this
     */
    public function tagFilter(string $fieldName, array $tags)
    {
        $queryTags = implode(' | ', $tags);
        $this->tagFilters[] = '@' . $fieldName . ':{' . $queryTags . '}';
    ...
    public function reduce(array $reducers)
    {
        ...
        foreach ($reducers as $alias => $reducer_data) {
            ...
            if (in_array($type, $valid_reducers)) {
                $reducer_data = array_map(function ($a) {
                    return "@$a";
                }, $reducer_data);
    ...
    public function searchQueryArgs($query)
    {
        ...
        $queryParts = array_merge((array)$query, $this->numericFilters, $this->geoFilters, $this->tagFilters);
